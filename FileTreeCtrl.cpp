...
{
	INT_PTR N=m_HidFolders.GetUpperBound();
...
CString CFileTreeCtrl::GetInfo(HTREEITEM hItem)
{
	...
	if (hItem)
	{
		...
		if (lptvid) {
            res.Format(
                _T("Files: %d;   Dirs: %d;   SelFiles: %d;   SelDirs: %d;   PartDirs: %d"),
...
INT_PTR CFileTreeCtrl::GetSelectedFiles(std::vector<CString> * files)
{
	INT_PTR N=SelFiles.GetCount();
...
INT_PTR CFileTreeCtrl::GetSelectedFolders(std::vector<CString> * folders)
{
	INT_PTR N=SelFolders.GetCount();
	...
	if (N) {
		...
		while (pos != NULL)
		{
			SelFolders.GetNextAssoc( pos, Key, (*folders)[i] );
			i++;
		}
	}
